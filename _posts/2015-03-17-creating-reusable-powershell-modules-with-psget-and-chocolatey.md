...
I also added a global call to Export-ModuleMember in order to export the cmdlets, functions and variables of the module. 
...
@{
	...
	FunctionsToExport = '*'
	CmdletsToExport = '*' # optional: CmdletsToExport = @('Join-Uri')
	VariablesToExport = '*'
	AliasesToExport = '*'
	FileList = @("Join-Uri.ps1", "IO.psm1")
}
```

I removed all of the additional comments in the module definition in order to keep the show the important information.
...
I also changed the FileList to include the Join-Uri.ps1 and IO.psm1 files and updated the RootModule to point to 'IO.psm1'.

I played around with the CmdletsToExport and Export-ModuleMember and it seems that you must use Export-ModuleMember in order to actually see the cmdlets exported. 
The CmdletsToExport in the module manifest will then act as an override. 
'*' says "Export everything the module (psm1 file) exports."
Putting a list of cmdlets in the CmdletsToExport would say "From what the module exports, only actually export what I have listed in CmdletsToExport"
...
The module manifest is valid powershell and you can test if the quality of the manifest by using the "Test-ModuleManifest" cmdlet. 

```powershell
Test-ModuleManifest 'IO.psd1'
